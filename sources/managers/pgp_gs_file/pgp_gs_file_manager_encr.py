...
def pgp_gs_files(file_list, gs_params: GsParams, pgp_params: PGPParams,db_manager: database_manager.DatabaseManager):
    try:
        filename_full = None
...
def main():
    ...
    if not files:
        ...
        sys.exit(0)
    try:
        pgp_gs_files(files,gs_params,pgp_params, db_manager)
        db_manager.close_step_log(input_data.workflow_name, input_data.step_name, input_data.work_flow_log_id,
                                  gs_params.work_flow_step_log_id, "success",
                                  f"all_gs_pgp_encr")
    except Exception as e:
