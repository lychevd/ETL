...
from core import gcs_manager, secret_manager, gsutil_manager, local_util, mysql_bulk_manager,mysql_sql_client_manager
...
class MySqlParamsOut:
    ...
    convert_to_parquet: str
    header: str
    expected_col_count: str
...
def create_gs_file_from_ms_sql(gc_params_in : GcParamsIn,my_sql_params_out:MySqlParamsOut,
    ...
    try:
        ...
        try:
            my_sql_bcp_manager_sql = mysql_bulk_manager.MySqlImportExportManager()
        ...
        try:
            ...
            raise
        try:
            mmysql_sql_client= mysql_sql_client_manager.MySQLClient(
                my_sql_params_out.mysql_host,my_sql_params_out.mysql_database, my_sql_params_out.mysql_user, my_sql_params_out.mysql_password,int(my_sql_params_out.mysql_port)
            )
        ...
        if my_sql_params_out.requires_boundary == "Y":
            try:
                End_boundary = mmysql_sql_client.execute_query_autocommit(my_sql_params_out.end_boundary_query, "Y")
        ...
        output_file = os.path.join(my_sql_params_out.local_data_directory, my_sql_params_out.filename_base)
        if my_sql_params_out.requires_boundary == "Y":
        ...
        row_count= mmysql_sql_client.export_query_to_file  ( query_adjusted,output_file,my_sql_params_out.data_file_delimiter, my_sql_params_out.row_delimiter, my_sql_params_out.header)
        ...
        if row_count == 0:
            ...
            sys.exit(0)
        if my_sql_params_out.convert_to_parquet=="Y":
            parquet_path = local.convert_txt_to_parquet_auto_ext(output_file,my_sql_params_out.data_file_delimiter)
        ...
        if gc_params_in.clean_local_dir == "Y":
            local.clean_local_directory(my_sql_params_out.local_data_directory)
    except Exception as err:
...
def main():
    ...
    my_sql_params_out = MySqlParamsOut(
        ...
        convert_to_parquet=secrets.get_variable_value( 'convert_to_parquet', variables, from_secret_list),
        ...
        expected_col_count=secrets.get_variable_value('expected_col_count', variables, from_secret_list)
    )
    ...
    if gc_params_in.get_key_file_from_sec == "Y":
        try:
            ...
            sys.exit(1)
    create_gs_file_from_ms_sql(gc_params_in,my_sql_params_out, db_manager)
    db_manager.close_step_log(input_data.workflow_name, input_data.step_name, input_data.work_flow_log_id,
