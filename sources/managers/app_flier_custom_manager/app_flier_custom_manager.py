...
def download_appsflyer_csv_with_windows(
    ...
    try:
        ...
        while current_start < end_ts:
            ...
            logging.info(f"current_start={current_start}")
            logging.info(f"current_end={current_end}")
...
def get_clean_date_range(raw_value: str, minutes_offset_start: int = 1, minutes_offset_end: int = 3) -> tuple[str, str]:
    """
    Cleans a datetime string and returns formatted start and end datetime strings.

    Args:
        raw_value (str): The raw datetime string (e.g., from BigQuery or API).
        minutes_offset_start (int): Minutes to add to the cleaned start date.
        minutes_offset_end (int): Minutes to subtract from current time for the end date.

    Returns:
        tuple[str, str]: A tuple of (start_date, end_date) in "%Y-%m-%d %H:%M:%S" format.
    """
    dt_format = "%Y-%m-%d %H:%M:%S"

    try:
        # Remove tabs, newlines, control characters, etc.
        cleaned = re.sub(r"[^\x20-\x7E]", "", raw_value)
        # Remove milliseconds or anything after the first 19 characters (standard datetime length)
        cleaned = cleaned.strip()[:19]

        parsed_date = datetime.strptime(cleaned, dt_format)
        start_date = (parsed_date + timedelta(minutes=minutes_offset_start)).strftime(dt_format)
        end_date = (datetime.now() - timedelta(minutes=minutes_offset_end)).strftime(dt_format)
        logging.info(f"Start_date={ start_date}")
        logging.info(f"end_date={end_date}")
        return start_date, end_date

    except Exception as e:
        raise ValueError(f"Failed to parse date '{raw_value}': {e}")
...
def main(param: InputParams):
    ...
    raw_value = bq.run_query_and_return_single_value(bq_params.query)
    start_date, end_date = get_clean_date_range(raw_value)

    additional_fields = None if api_param.additional_fields.lower() == 'none' else api_param.additional_fields
